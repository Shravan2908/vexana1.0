...
from vexana import dispatcher as d
...
class VexanaTelegramHandler:
    ...
    def message(self, pattern: Optional[str] = None, can_disable: bool = True, run_async: bool = True, group: Optional[Union[int]] = 60, friendly = None):
        def _message(func):
            try:
                ...
                log.debug(f"[Vexmsg] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(f"[Vexmsg] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            log.debug(f'[VexCALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
    ...
    def inlinequery(self, pattern: Optional[str] = None, run_async: bool = True, pass_user_data: bool = True, pass_chat_data: bool = True, chat_types: List[str] = None):
        def _inlinequery(func):
            ...
            log.debug(f'[VexINLINE] Loaded inlinequery handler with pattern {pattern} for function {func.__name__} | PASSES USER DATA: {pass_user_data} | PASSES CHAT DATA: {pass_chat_data} | CHAT TYPES: {chat_types}')
            return func
        return _inlinequery

Vexcmd = VexanaTelegramHandler(d).command
Vexmsg = VexanaTelegramHandler(d).message
Vexcallback = VexanaTelegramHandler(d).callbackquery
Vexinline = VexanaTelegramHandler(d).inlinequery
